...
""".strip()
    ...
    def override_language_by_keywords(text: str, initial_lang: str) -> str:
        ...
        if initial_lang in ('und', 'eng'):
            ...
            if any(ch in text for ch in 'éèêàçùôâîû'):       return "fre"
            if any(ch in text for ch in 'ñáíóú'):             return "spa"
            if any(ch in text for ch in 'ãõ'):                return "por"
        return initial_lang
    ...
    def is_literature_category(self, category_text: str) -> bool:
        ...
        text_match = self._has_kw(tokens, ko_hits) or self._has_kw(tokens, en_hits)
        if text_match:
            return True
        # KDC 8xx — '외국도서>문학' 또는 '국내도서>문학>한국문학' 형태로 변환된 값도 처리
        # (원본 KDC 번호가 그대로 남아있는 경우 대비)
        raw = category_text or ""
        kdc_match = bool(re.search(r"\b8\d{2}", raw))
        if kdc_match:
            self._dbg(f"🔎 [판정근거] KDC 8xx 번호 감지 → 문학 판정")
        return kdc_match

    @staticmethod
    def is_literature_top(category_text: str) -> bool:
        """'소설/시/희곡' 또는 KDC 변환 후 '>문학' 최상위 경로 감지."""
        cat = category_text or ""
        return "소설/시/희곡" in cat or ">문학" in cat or cat.endswith(">문학")

    def is_nonfiction_override(self, category_text: str) -> bool:
        """
        겉보기에는 문학이어도 역사·에세이·사회과학 등 비문학 키워드가
        있으면 비문학으로 강제 처리.
        단, 문학 최상위(소설/시/희곡)이면 '과학/기술'은 제외(SF 보호).
        """
        tokens  = self.tokenize_category(category_text or "")
        lit_top = self.is_literature_top(category_text or "")

        ko_nf = ["역사","근현대사","서양사","유럽사","전기","평전",
                 "사회","정치","철학","경제","경영","인문","에세이","수필"]
        en_nf = ["history","biography","memoir","politics","philosophy",
                 "economics","science","technology","nonfiction","essay","essays"]
        sci   = ["과학", "기술"]
        sci_en = ["science", "technology"]

        k = self._trigger_kw(tokens, ko_nf) or self._trigger_kw(tokens, en_nf)
        if k:
            self._dbg(f"🔎 [판정근거] 비문학 키워드 발견: '{k}'")
            return True

        if not lit_top:
            k2 = self._trigger_kw(tokens, sci) or self._trigger_kw(tokens, sci_en)
            if k2:
    ...
    def is_domestic_category(category_text: str) -> bool:
        """
        '국내도서' 텍스트 또는 KDC 한국문학(810.x) 감지 시 True.
        KDC 변환 후에는 '국내도서>문학>한국문학' 형태가 되므로 텍스트 매칭으로 충분.
        """
        cat = category_text or ""
        return "국내도서" in cat or bool(re.search(r"\b810", cat))
    ...
    def determine_h_language(
        ...
        문학   : 카테고리/웹 → 원제 유니코드 → GPT → 저자기반
        비문학 : GPT → 카테고리/웹 → 원제 유니코드 → 저자기반
        """
        ...
        if is_lit:
            # 문학 흐름: 1) 카테고리/웹 → 2) 원제 유니코드 → 3) GPT → 4) 저자기반
            lang_h = subject_lang or rule_from_original
            self._dbg(f"📘 [문학 흐름] 1차 후보: {lang_h or 'und'}")
            if not lang_h or lang_h == "und":
                self._dbg("📘 [문학 흐름] GPT 보완 시도…")
                lang_h = self.gpt_guess_original_lang(
                    title, category_text, publisher, author, original_title
                )
                self._dbg(f"📘 [문학 흐름] GPT 결과: {lang_h}")
            if (not lang_h or lang_h == "und") and author:
                self._dbg("📘 [문학 흐름] 저자 기반 보정 시도…")
                author_hint = self.gpt_guess_original_lang_by_author(
                    author, title, category_text, publisher
                )
                self._dbg(f"📘 [문학 흐름] 저자기반 결과: {author_hint}")
        else:
            # 비문학 흐름: 1) GPT → 2) 카테고리/웹 → 3) 원제 유니코드 → 4) 저자기반
            self._dbg("📘 [비문학 흐름] GPT 선행 판단…")
            lang_h = self.gpt_guess_original_lang(
                title, category_text, publisher, author, original_title
            )
            self._dbg(f"📘 [비문학 흐름] GPT 결과: {lang_h or 'und'}")
            if not lang_h or lang_h == "und":
                lang_h = subject_lang or rule_from_original
                self._dbg(f"📘 [비문학 흐름] 보조 규칙 적용 → 후보: {lang_h or 'und'}")
            if author and (not lang_h or lang_h == "und"):
                self._dbg("📘 [비문학 흐름] 저자 기반 보정 시도…")
                author_hint = self.gpt_guess_original_lang_by_author(
                    author, title, category_text, publisher
                )
                self._dbg(f"📘 [비문학 흐름] 저자기반 결과: {author_hint}")
        ...
        self._dbg("📘 [결과] 조정 후 원서 언어(h) =", lang_h)
    ...
    def get_kormarc_tags(
        ...
        """
        서지정보 dict(item)와 detail dict로부터
        041 · 546 필드 문자열과 원제를 반환.

        item['categoryText']는 알라딘 텍스트 형태 또는
        KDC 번호를 kdc_to_category_text()로 변환한 형태 모두 허용.
