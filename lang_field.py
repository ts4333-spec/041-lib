...
""".strip()
    ...
    def is_literature_category(self, category_text: str) -> bool:
        ...
        if self._has_kw(tokens, ko_hits) or self._has_kw(tokens, en_hits):
            return True
        # KDC 변환 후 '외국도서>문학>영미문학' 형태 대응
        return "문학" in (category_text or "")

    @staticmethod
    def is_literature_top(category_text: str) -> bool:
        cat = category_text or ""
        return "소설/시/희곡" in cat or ">문학" in cat
